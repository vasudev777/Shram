...
if (isset($_POST['submit'])) {
    ...
    // DB Insert
    $sql = "INSERT INTO build_cust_book(cust_id,b_id,build_book_date,build_book_note,build_book_status)
            VALUES ('$cid','$bid','$date','$note','0')";

    if (mysqli_query($conn, $sql)) {

        // Builder Details
        $builderResult = mysqli_query($conn, "SELECT * FROM build_details WHERE b_id='$bid'");
        $builder       = mysqli_fetch_assoc($builderResult);
        ...
        $bookingDetails = [
            'name'       => $builder['b_name'],
            'email'      => $builder['b_email'],
            'number'     => $builder['b_number'],
            'experience' => $builder['b_experience'],
            'city'       => $builder['b_city'],
            'date'       => $date,
            'note'       => $note,
        ];
        ...
        $customerBody = bookingEmailTemplate('builder', $bookingDetails, $cust['cust_name'], 'Pending');
        sendBookingEmail($cust['cust_email'], $cust['cust_name'], ' Builder Appointment Confirmed - Shram', $customerBody);

        echo "<script>alert('Appointment Booked! Confirmation email sent.'); window.location='builder_history.php';</script>";

    } else {
        echo "<script>alert('Appointment Not Booked!'); window.location='builder.php';</script>";
    }
}
?>